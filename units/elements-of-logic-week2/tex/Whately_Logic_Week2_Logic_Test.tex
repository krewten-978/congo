...
\textbf{Once the word is fixed, does the conclusion follow? Why or why not?}\studentline\studentline
}
...
\begin{quote}
``The trial was fair. But the sky was not fair that day, for it rained. Therefore the trial was not fair.''
\end{quote}
...
\section*{Part I: Find the Two-Faced Word}
For each notice, identify the term that changes meaning. Then decide whether the conclusion follows once the term is fixed.

\Needspace{0.30\textheight}
\subsection*{1. The Notice of the Fair Judge}
\textbf{Argument:} ``The trial was fair. But the sky was not fair that day, for it rained. Therefore the trial was not fair.''
\twomeaningsblock
...
\section*{Part II: Fix the Term}
Choose one notice from Part I. Rewrite the argument so that the key term is used in only one clear sense. Then test the repaired argument.

\textbf{Notice chosen:} \rule{1.8in}{0.4pt}

\textbf{Fixed meaning of the key term:}\studentline
\textbf{Rewritten argument:}\studentline\studentline\studentline
\textbf{Does the rewritten argument prove its conclusion? If not, what hidden assumption or missing fact remains?}\studentline\studentline
